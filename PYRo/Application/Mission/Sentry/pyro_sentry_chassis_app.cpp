...
extern "C"
{
    void chassis_rxcmd(void const *rc_ctrl)
    {
        ...
        rud_cmd_ptr->mode = static_cast<pyro::cmd_base_t::mode_t>(raw_data[3]);
        yaw_cmd_ptr->target_yaw_imu_angle =
            -static_cast<float>(static_cast<int8_t>(raw_data[4]));
        yaw_cmd_ptr->mode = static_cast<pyro::cmd_base_t::mode_t>(raw_data[5]);
    }
    ...
    {
        can_rx_drv_t::subscribe(pyro::can_hub_t::which_can::can2, 0x101);
